...
- lifecycle HTTP 任务默认持久化到 `data_lake/runtime/tasks.json`；`/api/training/runs`、`/api/evaluation/runs`、`/api/deployments`、`/api/autolabel/jobs` 至少会跨重启保留 `queued/canceled` 等基础状态，但当前仍是 JSON MVP，还没有接入真实 Python worker 调度。
