...
- Data Intake Plan 的 dry-run 构造已拆到 `internal/app/intakeapp`：runtime 的 `intake.plan` / `vlm.inspect` handler 只调用 intake app，并把 `plan_id`、`dataset_name`、`source_uri` 和审批边界写入 trace metadata。
