...
Then 未注册 tool、未知参数、高风险审批缺失会先被 `internal/app/toolapp` 拦截，合法 tool-call 才进入具体执行逻辑。

证据：`internal/app/toolapp/schema_test.go` 和 `internal/app/agentruntime/service_test.go`。
