...
Then 返回该 job 的生命周期日志、状态和进度；`/logs/stream` 以 NDJSON 输出已有日志和带 retry/heartbeat/artifact/manifest/stdout/stderr 摘要的终态事件；worker 运行中的 heartbeat 与 `stdout>` / `stderr>` 行也会持续写回并出现在同一条流中；Web 中点击 model job 能看到日志列表；交互式 CLI 可用 `/follow-job <job_id>` 跟随日志流直到终态或超时。
...
Then job 最终应把 worker heartbeat、artifact、attempt/max_attempts、retryable、stdout/stderr 摘要和 worker logs 写回现有 ModelJobStore，CLI/Web/API 读取同一份数据；worker 执行期间的 heartbeat 与结构化 `stdout/stderr` 行也应持续写回 store 并进入 `/logs/stream`；worker timeout、坏 JSON 和显式 failed 结果也应保留部分 stdout/stderr 并标记错误类型；当使用 JSON runtime store 时，还应把 artifact 摘要归档到 `runtime-root/artifacts/<job_id>.artifact_manifest.json`，归档文件采用 `artifact-manifest/v1` 并包含 `artifact_summary`，同时在 logs metadata 中暴露该路径；当设置 `AGENT_RUNTIME_HF_DOWNLOAD_RUNNER=service` 时，可显式回退旧下载 service runner，而 `model.verify_hf` / `model.smoke_locateanything` 默认仍保持同步模式，避免打断现有 smoke 链。
...
Then 已创建任务仍可通过同一 task id 查询到，且 `task_000001`、`task_000002` 这类序号不会在重载后从头开始；worker-backed lifecycle task 还应把 `running/completed/failed/canceled`、heartbeat、logs、artifacts、stdout/stderr 写回同一份 task store，并通过 `GET /api/tasks/{id}/logs` / `/logs/stream` 暴露同一份状态，`metadata.artifact_manifest` 指向归档后的 `artifact-manifest/v1` 文件。
