...
Given `model.download_hf` 收到 `dry_run=true` 或真实下载请求，或 `model.verify_hf` 收到 `job=true`
When Go `GoToolExecutor` 创建 `ModelJob` 并启动 `python -m agent_worker.main`
Then job 最终应把 worker heartbeat、artifact、attempt/max_attempts、retryable、stdout/stderr 摘要和 worker logs 写回现有 ModelJobStore，CLI/Web/API 读取同一份数据；当设置 `AGENT_RUNTIME_HF_DOWNLOAD_RUNNER=service` 时，可显式回退旧下载 service runner，而 `model.verify_hf` 默认仍保持同步模式，避免打断现有 smoke 链。
...
| ATDD-019 | 每次提交前执行 | rg + git status |
