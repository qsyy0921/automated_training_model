...
### ATDD-018 Go ModelJob 接入 Python worker dry-run

Given `model.download_hf` 收到 `dry_run=true`
When Go `GoToolExecutor` 创建 `ModelJob` 并启动 `python -m agent_worker.main`
Then job 最终应把 worker heartbeat、artifact、attempt/max_attempts、retryable、stdout 摘要和 worker logs 写回现有 ModelJobStore，CLI/Web/API 读取同一份数据。
