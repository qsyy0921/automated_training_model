...
- Python worker 已有 heartbeat、logs、retries、artifacts 的最小 health/job 契约，且 `model.download_hf` 默认、`model.verify_hf` 显式 `job=true`、`model.smoke_locateanything` 显式 `job=true`、`training.run(dry_run)` 已接入 Go `ModelJob`；lifecycle HTTP 任务也已进入 `WorkerGateway` 的通用 Python worker dry-run，并提供 task logs / stream / artifact manifest。后续仍需把真实训练/评估统一接到 task repository 和失败重试调度。
