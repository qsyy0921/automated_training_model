...
| Lifecycle Task Queue | `internal/infrastructure/queue/json.go`、`internal/infrastructure/modelgateway/worker.go`、`internal/app/lifecycleapp` | `/api/autolabel/jobs`、`/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 默认持久化到 `runtime-root/tasks.json`，并通过 Python worker 回写 `running/completed/failed/canceled/interrupted`、heartbeat、logs、artifacts、stdout/stderr、attempt/max_attempts、retry metadata；`dry_run=true` 输出显式 `*.plan`，`dry_run=false` 默认执行 repo-owned `execution_recipe=default` 并写出 `request.json` / `plan.json` / `result.json` / `recipe_spec.json` / `recipe_report.json`，同时调用 `workers/python/lifecycle_recipes/*` 生成实际 `generated/*` 产物（如训练 summary/metrics/checkpoint stub、评估 report、部署 manifest），这些生成文件也会作为 artifacts 暴露；显式提供 `execution_command` 时仍可切到 operator command 执行；服务重启前未完成的 task 会恢复为 `interrupted + resumable=true`，可通过 `POST /api/tasks/{id}/resume` 重新排队；`GET /api/tasks/{id}/logs` 与 `/logs/stream` 读取同一份 store，artifact 摘要按 `artifact-manifest/v1` 归档到 `runtime-root/artifacts/<task_id>.artifact_manifest.json`，包含 `artifact_summary`（artifact_count / role_counts / kind_counts / execution_mode_counts / primary_artifact） | 还不是统一 task repository；还没有真实 GPU 训练/部署 recipe、自动恢复 |
...
- `GET /api/runtime/model-jobs/{job_id}/logs/stream`：以 NDJSON 输出已有日志、运行中 `update` 事件和终态事件；当前 worker 运行中的 progress、heartbeat 与结构化 `stdout/stderr` 行会通过写回 store 持续推到这条流，后续再补原始字节级直通。
...
- `GET /api/tasks/{task_id}/logs/stream`：以 NDJSON 输出 lifecycle task 的已有日志、运行中 `update` 事件和终态事件；当前通过轮询同一份 task store 暴露运行中的 worker 输出。
...
- `labelctl agent` 支持 `/job-manifest <job_id>`、`/job-lineage <job_id>`、`/task-manifest <task_id>`、`/task-lineage <task_id>`、`/resume-task <task_id>`；一次性 CLI 支持 `labelctl runtime job-manifest|job-lineage|task-manifest|task-lineage|resume-task`、`labelctl models job-manifest|job-lineage`、`labelctl logs job-manifest|job-lineage|task-manifest|task-lineage` 和各 lifecycle 命令组的 `task-manifest|task-lineage|resume-task`。
...
- Tool runner 分发已迁移到 `internal/app/toolapp`；`intake.plan` / `vlm.inspect` 的 quarantine/scan/plan/workflow 构造已迁移到 `internal/app/intakeapp`，并通过 `internal/infrastructure/intakerepo.JSONRepository` 写入 `runtime-root/intake/intake_plans.json` 和 `intake_workflows.json`；`workflow.list_runs` / `workflow.submit_run` 的 dry-run 规则和 RunRequest 构造已迁移到 `internal/app/runtimeworkflow`；`model.download_hf` / `model.verify_hf` / `model.smoke_locateanything` 的参数规范化、路径安全、脚本执行和 smoke 解析已迁移到 `internal/app/modelruntime`。lifecycle HTTP 任务队列已从纯内存提升到 `runtime-root/tasks.json` 的 JSON MVP，并通过 `WorkerGateway` 进入 Python worker；`dry_run=true` 走 recipe plan，`dry_run=false` 默认执行 repo-owned recipe runner。task logs / NDJSON stream 与 artifact manifest 归档已接入同一份 task store。后续仍需把 model job 生命周期和 workflow run 接入正式 task repository。
- QQ 真实账号群聊 @Bot 实测。
- CLI / Gateway 的复杂 planner 分步流式、审批确认、model job 日志流和会话恢复；最小 tool progress streaming 已完成。
- Python worker 已有 heartbeat、logs、retries、artifacts 的最小 health/job 契约，且 `model.download_hf` 默认、`model.verify_hf` 显式 `job=true`、`model.smoke_locateanything` 显式 `job=true`、`training.run(dry_run)` / `evaluation.run(dry_run)` / `deployment.run(dry_run)` 已接入 Go `ModelJob`；lifecycle HTTP 任务也已进入 `WorkerGateway` 的 Python worker，`dry_run=true` 产出显式 `*.plan`，`dry_run=false` 默认执行 repo-owned `execution_recipe=default` 并写出 `recipe_spec.json` 与 `recipe_report.json`，显式提供 `execution_command` 时会执行真实命令，并提供 task logs / stream / artifact manifest。后续仍需把真实训练/评估统一接到 task repository 和失败重试调度。
