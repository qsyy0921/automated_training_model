...
- `labelctl agent` 支持 `/job-manifest <job_id>`、`/task-manifest <task_id>`；一次性 CLI 支持 `labelctl runtime job-manifest|task-manifest`、`labelctl models job-manifest`、`labelctl logs job-manifest|task-manifest` 和各 lifecycle 命令组的 `task-manifest`。
