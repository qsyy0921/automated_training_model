...
| Lifecycle Task Queue | `internal/infrastructure/queue/json.go`、`internal/app/lifecycleapp` | `/api/autolabel/jobs`、`/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 的基础任务元数据默认持久化到 `runtime-root/tasks.json` | 还不是统一 task repository；没有真实 worker、日志、artifact、retry |
...
- Tool runner 分发已迁移到 `internal/app/toolapp`；`intake.plan` / `vlm.inspect` 的 quarantine/scan/plan/workflow 构造已迁移到 `internal/app/intakeapp`，并通过 `internal/infrastructure/intakerepo.JSONRepository` 写入 `runtime-root/intake/intake_plans.json` 和 `intake_workflows.json`；`workflow.list_runs` / `workflow.submit_run` 的 dry-run 规则和 RunRequest 构造已迁移到 `internal/app/runtimeworkflow`；`model.download_hf` / `model.verify_hf` / `model.smoke_locateanything` 的参数规范化、路径安全、脚本执行和 smoke 解析已迁移到 `internal/app/modelruntime`。lifecycle HTTP 任务队列也已从纯内存提升到 `runtime-root/tasks.json` 的 JSON MVP。后续仍需把 model job 生命周期和 workflow run 接入正式 task repository。
- QQ 真实账号群聊 @Bot 实测。
