...
- lifecycle HTTP task queue 的恢复和真实 recipe 调度测试；当前已覆盖 `tasks.json` 持久化、worker-backed dry-run 状态流转、heartbeat/logs/stdout/artifact 回写、artifact manifest、task logs / NDJSON stream 和取消，但不覆盖真实训练/评估/部署 recipe。
