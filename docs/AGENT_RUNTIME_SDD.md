...
| Training Runtime | 当前先通过 `training.run(dry_run)` 进入 Python worker `ModelJob`，由 `/bot-train-dry <dataset_id> [target_task] [model_family]` 触发，返回 dry-run artifact 和 heartbeat；lifecycle HTTP 入口则把任务元数据先写入 `data_lake/runtime/tasks.json` | 后续接入真实训练 recipe、GPU 调度、checkpoint artifact 和统一 task repository |
