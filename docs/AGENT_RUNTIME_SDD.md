...
- Runtime execution worker smoke 已通过：`smoke-runtime-execution-worker.ps1` 通过 `/bot-train-run`、`/bot-eval-run`、`/bot-deploy-run` 直接触发 `dry_run=false` 的 Python worker `ModelJob`，默认执行 `execution_recipe=default`，并把 `request/plan/result/recipe_spec/recipe_report`、trace、job heartbeat 和 artifacts 写回同一份 runtime store。
- Lifecycle execution worker smoke 已通过：`smoke-lifecycle-execution-worker.ps1` 通过 `/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 提交带 `execution_recipe=default` 的 `dry_run=false` task，worker 会真实执行 repo-owned recipe runner，并写出 `request.json`、`plan.json`、`result.json`、`recipe_spec.json`、`recipe_report.json`，`/api/tasks/{id}/logs` 可读回 heartbeat、artifacts、stdout/stderr 和 artifact manifest。
