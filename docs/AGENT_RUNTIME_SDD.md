...
| Runtime Workflow | `internal/app/runtimeworkflow` 管理 `workflow.list_runs` / `workflow.submit_run` 的 dry-run 规则、RunRequest 构造和回复格式 | 后续替换为统一 workflow/task repository，并补日志和 artifacts |
...
| Training Runtime | 当前通过 `training.run`、`evaluation.run`、`deployment.run` 进入 Python worker `ModelJob`：`/bot-train-dry|run <dataset_id> [target_task] [model_family]`、`/bot-eval-dry|run <dataset_id> <model_id> [split]`、`/bot-deploy-dry|run <model_id> <target> [runtime] [replicas]` 均由 Go fast-path 直接创建后台 worker job；`*-dry` 返回 dry-run plan artifact 和 heartbeat，`*-run` 默认执行 repo-owned `execution_recipe=default` 并落地 `request/plan/result/recipe_spec/recipe_report`，其中 `recipe_spec.json` 记录 stage、outputs 和 repo-owned command preview；随后会调用 `workers/python/lifecycle_recipes/*` 生成 `generated/*` 产物，例如训练 summary/metrics/checkpoint stub、评估 report、部署 manifest。lifecycle HTTP 入口则通过 `WorkerGateway` 把 training/evaluation/deployment/autolabel 请求接成 Python worker task，并持久化到 `data_lake/runtime/tasks.json`；`dry_run=true` 产出显式 recipe plan，`dry_run=false` 同样默认执行 repo-owned recipe runner，显式提供 `execution_command` 时则会真实执行命令；可通过 `/api/tasks`、`/api/tasks/{id}`、`/api/tasks/{id}/logs` / `logs/stream`、`/api/tasks/{id}/lineage` 查看列表、详情、logs、artifact manifest 与 resume lineage。服务重启前仍处于 `pending/running` 的 lifecycle task 会在 JSON queue reload 时转成 `interrupted + resumable=true`，并通过 `POST /api/tasks/{id}/resume`、CLI `resume-task` 和 Web 按钮重新排队相同 payload；新 task 会记录 `parent_id / resumed_from_task_id`，旧 task 会记录 `resumed_by_task_id`。CLI 侧已对齐 `/tasks`、`/task`、`/task-logs`、`/task-lineage`、`/resume-task`、`/follow-task` 和 `labelctl runtime/logs/...`，Web Agent Overview 也已对齐 task 列表、日志和 lineage 面板 | 后续接入真实训练 recipe、GPU 调度、checkpoint artifact、自动恢复策略和统一 task repository |
...
- Runtime evaluation dry-run worker smoke 已通过：`smoke-evaluation-dry-worker.ps1` 通过 `/bot-eval-dry shanghaitech-original model-1 validation` 直接触发 `evaluation.run` 的 Python worker dry-run，trace、job heartbeat 和 recipe artifact 都能从同一份 runtime store 读回。
- Runtime deployment dry-run worker smoke 已通过：`smoke-deployment-dry-worker.ps1` 通过 `/bot-deploy-dry model-1 local-dry-run python-worker 2` 直接触发 `deployment.run` 的 Python worker dry-run，trace、job heartbeat 和 recipe artifact 都能从同一份 runtime store 读回。
- Runtime execution worker smoke 已通过：`smoke-runtime-execution-worker.ps1` 通过 `/bot-train-run`、`/bot-eval-run`、`/bot-deploy-run` 直接触发 `dry_run=false` 的 Python worker `ModelJob`，默认执行 `execution_recipe=default`，并把 `request/plan/result/recipe_spec/recipe_report`、`generated/*`、trace、job heartbeat 和 artifacts 写回同一份 runtime store。
- Lifecycle execution worker smoke 已通过：`smoke-lifecycle-execution-worker.ps1` 通过 `/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 提交带 `execution_recipe=default` 的 `dry_run=false` task，worker 会真实执行 repo-owned recipe runner，调用 `workers/python/lifecycle_recipes/*`，并写出 `request.json`、`plan.json`、`result.json`、`recipe_spec.json`、`recipe_report.json`、`generated/*`；`/api/tasks/{id}/logs` 可读回 heartbeat、artifacts、stdout/stderr 和 artifact manifest。
