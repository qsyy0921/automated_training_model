...
| Training Runtime | 当前先通过 `training.run(dry_run)` 进入 Python worker `ModelJob`，由 `/bot-train-dry <dataset_id> [target_task] [model_family]` 触发，返回 dry-run artifact 和 heartbeat；lifecycle HTTP 入口则通过 `WorkerGateway` 把 training/evaluation/deployment/autolabel 请求接成 Python worker dry-run task，并持久化到 `data_lake/runtime/tasks.json`，可通过 `/api/tasks/{id}/logs` / `logs/stream` 查看 logs 与 artifact manifest；其中训练/评估/部署/自动标注已产出显式 recipe plan artifact，而不是 generic dry-run 接受回执 | 后续接入真实训练 recipe、GPU 调度、checkpoint artifact 和统一 task repository |
