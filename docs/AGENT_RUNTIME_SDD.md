...
低延迟策略参考 ccb / Hermes / OpenClaw 的工程做法：明确命令不等待模型，项目身份不让模型自由发挥，高置信度工程意图先路由到受控 tool，长任务不阻塞入口，普通 chat 用流式首包改善体感，复杂任务才进入结构化 planner。当前 fast-path 覆盖 `/bot-ping`、`/bot-me`、`/bot-status`、`/bot-runs`、`/bot-run dry`、`/bot-verify-hf-job`、`/bot-train-dry`、`/bot-eval-dry`、`/bot-deploy-dry`、`/bot-help`、runtime self-description、`规划 ShanghaiTech 数据接入`、已知 LocateAnything 下载和 ShanghaiTech smoke 固定工具链；这些请求即使启用了 `AGENT_RUNTIME_USE_MIMO=true` 也不会等待 Python/Mimo。
...
| Training Runtime | 当前先通过 `training.run(dry_run)`、`evaluation.run(dry_run)`、`deployment.run(dry_run)` 进入 Python worker `ModelJob`，分别由 `/bot-train-dry <dataset_id> [target_task] [model_family]`、`/bot-eval-dry <dataset_id> <model_id> [split]`、`/bot-deploy-dry <model_id> <target> [runtime] [replicas]` 触发，返回 dry-run artifact 和 heartbeat；lifecycle HTTP 入口则通过 `WorkerGateway` 把 training/evaluation/deployment/autolabel 请求接成 Python worker task，并持久化到 `data_lake/runtime/tasks.json`，`dry_run=true` 产出显式 recipe plan，`dry_run=false` 默认执行 repo-owned `execution_recipe=default` 并落地 `request/plan/result/recipe_report`，显式提供 `execution_command` 时则会真实执行命令；可通过 `/api/tasks`、`/api/tasks/{id}`、`/api/tasks/{id}/logs` / `logs/stream` 查看列表、详情、logs 与 artifact manifest；CLI 侧已对齐 `/tasks`、`/task`、`/task-logs`、`/follow-task` 和 `labelctl runtime/logs/...`，Web Agent Overview 也已对齐 task 列表与日志面板 | 后续接入真实训练 recipe、GPU 调度、checkpoint artifact 和统一 task repository |
