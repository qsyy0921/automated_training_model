...
- Lifecycle execution worker smoke 已通过：`smoke-lifecycle-execution-worker.ps1` 通过 `/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 提交带 `execution_command` 的 `dry_run=false` task，worker 会真实执行命令，并写出 `request.json`、`plan.json`、`result.json`，`/api/tasks/{id}/logs` 可读回 heartbeat、artifacts、stdout/stderr 和 artifact manifest。
