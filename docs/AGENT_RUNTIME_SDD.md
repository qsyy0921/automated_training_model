...
- Lifecycle execution worker smoke 已通过：`smoke-lifecycle-execution-worker.ps1` 通过 `/api/training/runs`、`/api/evaluation/runs`、`/api/deployments` 提交 `dry_run=false`，task 会完成并写出 `request.json`、`plan.json`、`result.json` execution bundle，`/api/tasks/{id}/logs` 可读回 heartbeat、artifacts 和 artifact manifest。
