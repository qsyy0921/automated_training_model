...
本产品的长期定位是“小模型训练到部署”的完整数据闭环平台，覆盖数据接入、标注审核、自动标注、训练、评估、模型注册、部署发布和线上反馈回流。

当前阶段优先落地视频打标与审核，因为小模型性能首先取决于数据质量。第一版面向视频目标检测、追踪、分割、动作理解、异常检测等任务。它不只提供逐帧框选能力，还需要支持从已有 tracking 结果出发，完成轨迹审核、对象级异常事件标注、关键帧挑选、SAM/视频分割模型辅助传播、大模型辅助描述与一致性检查。
