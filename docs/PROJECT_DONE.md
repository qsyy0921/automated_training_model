...
- [x] 为 lifecycle worker dry-run 增加显式 recipe 计划：`training.run`、`evaluation.run`、`deployment.run`、`autolabel.run` 会解析 `request_json`、校验关键字段，并产出对应的 `*.plan` artifact 与摘要消息，而不是只返回 generic `dry-run accepted`。
...
- [x] 将 repo-owned lifecycle recipe runner 从“只落 spec/report”推进到“真实执行 repo 内脚本”：默认 `execution_recipe=default` 现在会调用 `workers/python/lifecycle_recipes/*`，生成 `generated/*` 产物（训练 summary/metrics/checkpoint stub、评估 report、部署 manifest、autolabel review queue），并把这些文件作为 artifacts 暴露给 CLI/Web/API；runtime 与 lifecycle execution smoke 已验证通过。
...
- [x] 为 lifecycle JSON task queue 增加重启恢复和手动 resume 语义：服务重启前仍处于 `pending/running` 的 task 会恢复为 `interrupted`、`resumable=true` 并写入 `interrupted_reason=server_restart`；新增 `POST /api/tasks/{id}/resume`、`labelctl runtime resume-task`、各 lifecycle 命令组 `resume-task`、交互式 CLI `/resume-task` 和 Web Agent Overview / Task Monitor Panel 的重新排队按钮，新 task 会记录 `parent_id` / `resumed_from_task_id`，旧 task 会记录 `resumed_by_task_id`。
- [x] 为 model job / lifecycle task 增加 lineage 查询：新增 `GET /api/runtime/model-jobs/{id}/lineage` 与 `GET /api/tasks/{id}/lineage`，按创建时间返回同一条 resume/recovery 链上的 parent/child 任务；CLI 新增 `runtime/models/logs job-lineage`、`runtime/logs/training/evaluation/deploy/autolabel task-lineage` 和交互式 `/job-lineage`、`/task-lineage`，Web Agent Overview 与共享 `TaskMonitorPanel` 也会展示当前选中 job/task 的 lineage，便于继续推进恢复策略、task repository 和 lineage catalog。
- [x] 为现有手动恢复/取消语义补最小幂等保证：重复 `resume-job` / `resume-task` 如果 parent 已经记录 `resumed_by_*` 且 child 仍存在，会直接返回同一个 child，不再重复排队；重复 `cancel-job` / `cancel-task` 对已取消项返回当前状态，不再把同一请求视为错误。
...
- [x] 为 `labelctl training/evaluation/deploy/autolabel submit` 增加 execution flags：`-exec`、`-exec-arg`、`-exec-cwd`、`-exec-env`、`-exec-timeout`、`-exec-recipe`；新增 `smoke-lifecycle-cli-execution-worker.ps1`，验证 CLI 可把 repo-owned recipe 或 execution command 透传到 Python worker。
- [x] 将 lifecycle `dry_run=false` 默认路径从单纯 execution bundle 提升到 repo-owned recipe runner：`training.run` / `evaluation.run` / `deployment.run` / `autolabel.run` 在未显式提供 `execution_command` 时会自动执行 `execution_recipe=default`，写出 `recipe_report.json` 并返回 `recipe-executed` / `recipe-failed` / `recipe-timeout`；HTTP 与 CLI 两条 recipe 路径都已 smoke 通过。
...
- [x] 将 runtime / lifecycle 的 artifact manifest 统一成 `artifact-manifest/v1`：`JSONModelJobStore` 与 `JSONQueue` 归档文件现在都包含 `schema_version`、`artifact_summary`（`artifact_count`、`role_counts`、`kind_counts`、`execution_mode_counts`、`primary_artifact`）和原始 `artifacts` 列表，为后续 lineage / Task Center / Model Registry 的消费契约固定基础格式。
- [x] 将 artifact manifest 从“只暴露路径”推进到可消费接口：新增 `GET /api/runtime/model-jobs/{id}/manifest` 与 `GET /api/tasks/{id}/manifest`；交互式 `labelctl agent` 支持 `/job-manifest`、`/task-manifest`，一次性 CLI 支持 `runtime/models/logs ...manifest` 和 lifecycle `task-manifest`；Web Agent Overview / Task Monitor Panel 现在会直接显示 artifact count 与 summary，而不只展示 manifest 文件路径。
...
- [x] 为 model job / lifecycle task 的 `logs/stream` 增加运行中 `update` 事件：当 progress、heartbeat、stdout/stderr 摘要、attempt/max_attempts 或 message 发生变化时，HTTP NDJSON 流会在终态前推送结构化 `update`，`labelctl agent /follow-job` 与 `/follow-task` 也会直接显示这些运行中状态变化。
