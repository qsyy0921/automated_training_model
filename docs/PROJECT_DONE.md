...
- [x] 为 model job / lifecycle task 增加 lineage 查询：新增 `GET /api/runtime/model-jobs/{id}/lineage` 与 `GET /api/tasks/{id}/lineage`，按创建时间返回同一条 resume/recovery 链上的 parent/child 任务；CLI 新增 `runtime job-lineage/task-lineage` 和交互式 `/job-lineage`、`/task-lineage`，Web Agent Overview 也会展示当前选中 job/task 的 lineage，便于继续推进恢复策略、task repository 和 lineage catalog。
