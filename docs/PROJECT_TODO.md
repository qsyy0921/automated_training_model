...
- [ ] 将 lifecycle task 的 interrupted/resume 从当前“手动重新排队”推进到真正的恢复闭环：当前 `tasks.json` 重启后会把 `pending/running` 标记为 `interrupted + resumable=true`，并可通过 `POST /api/tasks/{id}/resume` / CLI / Web 重新排队同一 payload，`/api/tasks/{id}/lineage` 已可查看 child task 链；后续仍需补自动 retry、resume 去重、cancel/resume 幂等和恢复策略审批。
