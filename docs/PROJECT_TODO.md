...
- [ ] 把 `training.run(dry_run)` 继续扩展到真实训练 recipe、evaluation/deployment 同类 worker job，并与统一 task repository、artifact manifest、checkpoint lineage 打通；当前 `/bot-train-dry`、`/bot-eval-dry`、`/bot-deploy-dry` 与 lifecycle HTTP task 已共享显式 dry-run recipe，但还不执行真实训练/评估/部署。
...
- [ ] 将 `lifecycleapp` 当前的 JSON task queue + `WorkerGateway` MVP 升级为统一 task repository：补真实训练/评估/部署 recipe、恢复、审批和更细粒度的状态机，而不是只停留在 worker dry-run；当前 task logs / NDJSON stream 和 artifact manifest 归档已补齐。
