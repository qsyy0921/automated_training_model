...
func TestRuntimeRouterSelectsLocalControlForTrainingDryRunCommand(t *testing.T) {
	msg := channel.InboundMessage{ID: "msg1", Channel: channel.KindQQ, Text: "/bot-train-dry shanghaitech-original detection yolo11n"}
	intent := ClassifyIntent(msg)
	route := NewRuntimeRouter().Select(PlanRequest{Message: msg, Intent: intent})
	if route.Mode != RouteLocalControl {
		t.Fatalf("expected local control route, got %+v", route)
