...
func runAutoLabel(cfg Config, args []string) error {
	...
	case "task-lineage":
		if len(args) < 2 {
			return errors.New("usage: labelctl autolabel task-lineage <task_id>")
		}
...
func runTraining(cfg Config, args []string) error {
	...
	case "task-lineage":
		if len(args) < 2 {
			return errors.New("usage: labelctl training task-lineage <task_id>")
		}
		return getJSON(cfg.addr + "/api/tasks/" + url.PathEscape(args[1]) + "/lineage")
	case "follow-task":
		if len(args) < 2 {
...
func runEvaluation(cfg Config, args []string) error {
	...
	case "task-lineage":
		if len(args) < 2 {
			return errors.New("usage: labelctl evaluation task-lineage <task_id>")
		}
		return getJSON(cfg.addr + "/api/tasks/" + url.PathEscape(args[1]) + "/lineage")
	case "follow-task":
		if len(args) < 2 {
...
func runModels(cfg Config, args []string) error {
	...
	case "job-lineage":
		if len(args) < 2 {
			return errors.New("usage: labelctl models job-lineage <job_id>")
		}
...
func runDeploy(cfg Config, args []string) error {
	if len(args) == 0 || args[0] == "help" {
		return errors.New("usage: labelctl deploy submit -model <id> -target <target> [-version v] [-runtime runtime] [-exec-recipe name] [-exec cmd -exec-arg=arg -exec-cwd dir -exec-env KEY=VALUE -exec-timeout sec] | task <task_id> | task-logs <task_id> | task-manifest <task_id> | task-lineage <task_id> | follow-task <task_id> | resume-task <task_id> | cancel-task <task_id>")
	}
	switch args[0] {
	case "submit":
	...
	case "task-lineage":
		if len(args) < 2 {
			return errors.New("usage: labelctl deploy task-lineage <task_id>")
		}
		return getJSON(cfg.addr + "/api/tasks/" + url.PathEscape(args[1]) + "/lineage")
	case "follow-task":
		if len(args) < 2 {
...
func runLogs(cfg Config, args []string) error {
	...
	case "job", "job-logs", "job-manifest", "job-lineage":
		if len(args) < 2 {
