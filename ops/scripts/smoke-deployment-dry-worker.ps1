param(
  [string]$Addr = "127.0.0.1:7953",
  [string]$Go = "",
  [string]$ModelId = "model-1",
  [string]$Target = "local-dry-run",
  [string]$Runtime = "python-worker",
  [string]$Replicas = "2",
...
$out = Join-Path $tmpDir "smoke-deployment-dry-worker.out.log"
$err = Join-Path $tmpDir "smoke-deployment-dry-worker.err.log"
...
try {
  ...
  $command = "/bot-deploy-dry $ModelId $Target $Runtime $Replicas"
  ...
  $trace = @($traces.traces | Where-Object { $_.tool_ids -contains "deployment.run" } | Select-Object -First 1)
  Assert-True ($trace.Count -eq 1) "runtime trace missing deployment.run"

  $deadline = (Get-Date).AddMinutes($TimeoutMinutes)
  $jobID = ""
  while ((Get-Date) -lt $deadline) {
    $jobs = Invoke-JSON -Url ("{0}/api/runtime/model-jobs?limit=20" -f $baseURL) -TimeoutSec 10
    $job = @($jobs.jobs | Where-Object { $_.kind -eq "deployment.run" -and $_.metadata.model_id -eq $ModelId -and $_.metadata.target -eq $Target } | Select-Object -First 1)
    if ($job.Count -eq 1) {
      $jobID = $job[0].id
      ...
      if ($job[0].status -eq "failed") {
        throw "deployment dry-run worker job failed: $($job[0].error)"
      }
    }
    Start-Sleep -Seconds 2
  }
  Assert-True (-not [string]::IsNullOrWhiteSpace($jobID)) "deployment dry-run worker job was not created"

  $logs = Invoke-JSON -Url ("{0}/api/runtime/model-jobs/{1}/logs" -f $baseURL, $jobID) -TimeoutSec 30
  Assert-True ($logs.status -eq "succeeded") "deployment dry-run worker job did not succeed"
  Assert-True ($null -ne $logs.worker_heartbeat) "deployment dry-run logs missing worker heartbeat"
  Assert-True ($logs.worker_heartbeat.status -eq "completed") "deployment dry-run heartbeat not completed"
  Assert-True (($logs.artifacts | Measure-Object).Count -ge 1) "deployment dry-run artifacts missing"

  Write-Host "smoke-deployment-dry-worker passed"
} finally {
