...
$out = Join-Path $tmpDir "smoke-training-dry-worker.out.log"
$err = Join-Path $tmpDir "smoke-training-dry-worker.err.log"
...
try {
  ...
  $command = "/bot-train-dry $DatasetId $TargetTask $ModelFamily"
  ...
  $trace = @($traces.traces | Where-Object { $_.tool_ids -contains "training.run" } | Select-Object -First 1)
  Assert-True ($trace.Count -eq 1) "runtime trace missing training.run"

  $deadline = (Get-Date).AddMinutes($TimeoutMinutes)
  $jobID = ""
  while ((Get-Date) -lt $deadline) {
    $jobs = Invoke-JSON -Url ("{0}/api/runtime/model-jobs?limit=20" -f $baseURL) -TimeoutSec 10
    $job = @($jobs.jobs | Where-Object { $_.kind -eq "training.run" -and $_.metadata.dataset_id -eq $DatasetId } | Select-Object -First 1)
    if ($job.Count -eq 1) {
      $jobID = $job[0].id
      ...
      if ($job[0].status -eq "failed") {
        throw "training dry-run worker job failed: $($job[0].error)"
      }
    }
    Start-Sleep -Seconds 2
  }
  Assert-True (-not [string]::IsNullOrWhiteSpace($jobID)) "training dry-run worker job was not created"

  $logs = Invoke-JSON -Url ("{0}/api/runtime/model-jobs/{1}/logs" -f $baseURL, $jobID) -TimeoutSec 30
  Assert-True ($logs.status -eq "succeeded") "training dry-run worker job did not succeed"
  Assert-True ($null -ne $logs.worker_heartbeat) "training dry-run logs missing worker heartbeat"
  Assert-True ($logs.worker_heartbeat.status -eq "completed") "training dry-run heartbeat not completed"
  Assert-True (($logs.artifacts | Measure-Object).Count -ge 1) "training dry-run artifacts missing"

  Write-Host "smoke-training-dry-worker passed"
} finally {
